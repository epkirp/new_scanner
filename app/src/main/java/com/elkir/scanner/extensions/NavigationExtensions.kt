...
//обрабатывает ситуацию, когда при быстром нажатии фрагмент открывается несколько раз
fun NavController.safetyOpenFragment(targetFragmentId: Int, openAction: () -> Unit) {
    if (currentDestination?.id != targetFragmentId) {
        openAction.invoke()
    }
}
